...
export function validateContactPayload(payload) {
  ...
  if (!name || name.length < 2) {
    result.ok = false;
    result.errors.push("Introduceți un nume valid.");
  }

  if (!phone) {
    result.ok = false;
    result.errors.push("Introduceți un număr de telefon valid.");
  }

  if (payload?.guestCount && guestCount === null) {
    result.ok = false;
    result.errors.push("Introduceți un număr de persoane valid.");
  }

  if (isLikelySpam(result.values)) {
    result.ok = false;
    result.errors.push("Cererea a fost oprită de filtrul anti-spam.");
  }
...
export function validateEventPayload(payload) {
  ...
  if (email === false) {
    result.ok = false;
    result.errors.push("Introduceți o adresă de email validă.");
  }

  if (!eventType) {
    result.ok = false;
    result.errors.push("Selectați tipul evenimentului.");
  }

  if (!eventDate) {
    result.ok = false;
    result.errors.push("Introduceți data evenimentului.");
  }

  if (payload?.guestCount && guestCount === null) {
    result.ok = false;
    result.errors.push("Introduceți un număr valid de participanți.");
  }
